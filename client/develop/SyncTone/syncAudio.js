...
module.exports = (context) => {
    return new SyncAudio(context)
}

function SyncAudio(context, clientTime) {
...
SyncAudio.prototype.setAudioList = function(audioUrlList) {
    this.audioUrlList = audioUrlList
}


SyncAudio.prototype.getCurrentTime = function() {
    return this.context.currentTime
}

let finishLoad = () => {}
SyncAudio.prototype.loadBuffer = function(audioUrlList, callback = () => {}) {
...
SyncAudio.prototype.preConnect = function(destination, syncSound) {
    // to array
...
SyncAudio.prototype.speedPlay = function(syncSound) {
    // to array
    syncSound.start()
}


SyncAudio.prototype.play = function(destination, syncSound) {
    // to array
    if (!Array.isArray(syncSound)) {
        let temp = syncSound
        syncSound = []
...
SyncAudio.prototype.addBuffer = function(name, buffer, callback = () => {}) {
    let syncAudio = this
    context.decodeAudioData(buffer, function(decodedBuffer) {
        syncAudio.buffer[name] = decodedBuffer
        callback()
    }, (err) => {
        console.log(err)
    })
