...
exports.webAudioAPI = (_context, clientTime) => {
    ...
    load(audioUrlList, (bufferList) => {
        ...
        socket.on('touch_surround_note', (body) => {
            ...
            let date = new Date(correctionTime)
            let left = correctionTime - Date.now()
            log.innerHTML = name + '  start playback after: ' + left.toFixed(4) + 'ms'
            job(date, () => {
                console.log('start note')
                notePlay(buffer[name], position)
            })
        })
        notePlay(buffer['E3'], {
          x:0.5,
...
let notePlay = (buffer, position) => {
    ...
    let x = position.x * field.w - field.speaker.x
    let y = position.y * field.h - field.speaker.y
    let dx = x / field.size
    let dy = y / field.size
    console.log(dx,dy)
