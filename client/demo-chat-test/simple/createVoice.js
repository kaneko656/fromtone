...
Sound.prototype.create = function(query, callback) {
    let text = query.text ? query.text : ''
    let speaker = query.speaker ? query.speaker : this.voice.SPEAKER.HIKARI
    let emotion = query.emotion ? query.emotion : undefined
    ...
    request.open('POST', 'https://api.voicetext.jp/v1/tts', true)
    // POST 送信の場合は Content-Type は固定.
    request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded')
    request.setRequestHeader("Authorization", "Basic " + this.apiKey + ":")
    // request.setRequestHeader("Authorization", "Basic " + Base64.encode(this.apiKey + ":"))
    ...
    //         //     volume: volume
    //         // })
    //         // fs.writeFile(cachePath + '/cache.json', JSON.stringify(cache, null, '    '))
    //
    //
