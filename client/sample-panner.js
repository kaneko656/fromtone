...
function Field(canvas) {
    ...
    this.manIcon.src = 'lib/sample/man.svg';

    this.speakerIcon = new Image();
    this.speakerIcon.src = 'lib/sample/speaker.svg';
...
let context
module.exports = (element, _context, socket) => {
    context = _context
    let sample = new PositionSample(element, _context, socket)

}

function PositionSample(el, context, socket) {
    var url = 'lib/sample/position.wav'
    // var url = 'lib/sound/melodius-scale-piano-C3.wav'
    var sample = this;
    ...
    this.socket.on('play', (body) => {
        ...
        this.syncPlay(body.time)
    })

    window.addEventListener('click', () => {
        this.triggerSyncPlay()
    }, false);
}
...
PositionSample.prototype.triggerSyncPlay = function() {
    this.isSyncing = true
...
PositionSample.prototype.prePlay = function() {
    // Hook up the audio graph for this sample.
    var source = context.createBufferSource();
    source.buffer = this.buffer;
    source.loop = true;
...
PositionSample.prototype.startSyncPlay = function() {
    this.source.start(0);
    this.isPlaying = true
}

PositionSample.prototype.syncPlay = function(time) {
    if (this.isPlaying) {
        this.stop()
    }
    this.prePlay()
...
PositionSample.prototype.changePosition = function(position) {
    ...
    // with -0.5 < x, y < 0.5
    if (position) {
        if (!this.isPlaying && !this.isSyncing) {
            this.play();
        }
        if (this.isSyncing) {
            return
        ...
        this.socket.on('position', (body) => {
            this.panner.setPosition(body.x, body.y, -0.5);
        })
        // console.log({
        //     x: x * mul,
        //     y: y * mul
